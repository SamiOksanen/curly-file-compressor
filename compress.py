...
def compress(i, o):
    ...
    for bracket in brackets:
        if bracket + ':' in content:
            raise Exception('File containing "' + bracket + ':". Can not compress.')

        contents = replaceToCurlies(content, bracket)
        if len(contents[0]) > 0:
            suppressions = contents[0] + bracket + ':' + suppressions
            content = contents[1]
...
def replaceToCurlies(content, curly):
    ...
    dups = list(dict.fromkeys([x for x in dupsCombo if len(x) >= 1 and curly not in x]))
    print(dups)
    for dup in dups:
        ssdup = ' ' + dup + ' '
        ssdupCount = len(re.findall(re.escape(ssdup), content))
        ...
        sdup = dup + ' '
        sdupCount = len(re.findall(re.escape(sdup), content))
        ...
        dupCount = len(re.findall(re.escape(dup), content))
